...
// New endpoints for team subfolder files
router.get("/listTeamSubfolderFiles/:teamName/:subFolder", (req, res) => {
  const requestedTeam = decodeURIComponent(req.params.teamName)
    .replace(/[''‛`´]/g, "'")
    .replace(/\s+/g, " ")
    .trim()
...
router.get("/downloadTeamSubfolderFile/:teamName/:subFolder/:fileName", (req, res) => {
  const requestedTeam = decodeURIComponent(req.params.teamName)
    .replace(/[''‛`´]/g, "'")
    .replace(/\s+/g, " ")
    .trim()
