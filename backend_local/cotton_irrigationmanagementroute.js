...
//     // Insert a new row without checking for duplicates
//     await request.query(
//       "INSERT INTO [2025_[2025_cotton_soil_moisture_sensor_data]] (teamName, sensorType, date, reading, options, applied, dateToday, applicationType, sensorConfirmed, optionConfirmed) VALUES (@teamName, @sensorType, @date, @reading, @options, @applied, @dateToday, @applicationType, @sensorConfirmed, @optionConfirmed)"
//     );
...
//     // Query the database to fetch all soil moisture sensor data for the specified team
//     const result = await request.query(
//       "SELECT * FROM [2025_[2025_cotton_soil_moisture_sensor_data]] WHERE teamName = @teamName"
//     );
...
//     // Execute the query to fetch all soil moisture sensor data
//     const result = await request.query(
//       "SELECT * FROM [2025_[2025_cotton_soil_moisture_sensor_data]]"
//     );
...
//     // Perform the database delete operation
//     await request.query(
//       "DELETE FROM [2025_[2025_cotton_soil_moisture_sensor_data]] WHERE id = @appId"
//     );
...
//     // Perform the database update operation
//     await request.query(
//       "UPDATE [2025_[2025_cotton_soil_moisture_sensor_data]] SET applied = @newAppliedValue WHERE id = @appId"
//     );
...
//       // Check if a record already exists and update it, or insert a new one
//       const sqlQuery = `
//       IF EXISTS (SELECT 1 FROM [2025_[2025_cotton_soil_moisture_sensor_data]] WHERE teamName = @teamName AND applicationType = @applicationType)
//         UPDATE [2025_[2025_cotton_soil_moisture_sensor_data]] SET optionConfirmed = @optionConfirmed WHERE teamName = @teamName AND applicationType = @applicationType;
//       ELSE
//         INSERT INTO [2025_[2025_cotton_soil_moisture_sensor_data]] (teamName, applicationType, optionConfirmed) VALUES (@teamName, @applicationType, @optionConfirmed);
//     `;
...
//       // Execute the query
//       const result = await request.query(
//         `SELECT applicationType, optionConfirmed FROM [2025_[2025_cotton_soil_moisture_sensor_data]] WHERE teamName = @teamName`
//       );
...
//       // Use the MERGE statement to insert or update as necessary
//       const sqlQuery = `
//       MERGE INTO [2025_[2025_cotton_soil_moisture_sensor_data]] AS target
//       USING (SELECT @teamName AS teamName, @sensorType AS sensorType) AS source
//       ON target.teamName = source.teamName AND target.sensorType = source.sensorType
//       WHEN MATCHED THEN
//         UPDATE SET sensorConfirmed = @sensorConfirmed
//       WHEN NOT MATCHED THEN
//         INSERT (teamName, sensorType, sensorConfirmed)
//         VALUES (@teamName, @sensorType, @sensorConfirmed);
//     `;
...
//       // Execute the query
//       const result = await request.query(
//         `SELECT sensorType, sensorConfirmed FROM [2025_[2025_cotton_soil_moisture_sensor_data]] WHERE teamName = @teamName`
//       );
