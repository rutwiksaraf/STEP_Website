...
router.post("/register", async (req, res) => {
  ...
  const { cropType, teamMembers } = formData;

  delete formData.teamMembers; // Remove teamMembers from formData to avoid the "Invalid string" error
  ...
  try {
    ...
    checkRequest.input("teamName", sql.VarChar, formData.teamName);

    let checkSql;

    // Define SQL statement to check for existing team based on the crop type
    if (cropType === "corn") {
      checkSql = `SELECT * FROM corn_registration_data WHERE teamName = @teamName`;
    } else if (cropType === "cotton") {
      checkSql = `SELECT * FROM cotton_registration_data WHERE teamName = @teamName`;
    } else {
    ...
    Object.keys(formData).forEach((key) => {
      registrationRequest.input(key, formData[key]); // Adjust SQL data types as necessary
    });
    ...
    if (cropType === "corn") {
      ...
      `;
      teamMembersTable = "corn_team_members";
    } else if (cropType === "cotton") {
      registrationSql = `
        ...
        SELECT SCOPE_IDENTITY() AS teamId;
      `;
      teamMembersTable = "cotton_team_members";
    } else {
      return res.status(400).json({ message: "Unsupported crop type" });
    }
