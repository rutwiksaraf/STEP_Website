...
const FAWN_WEATHER_API_URL =
  "https://fawn.ifas.ufl.edu/controller.php/week/obs/170;json";
...
const fetchETData = async () => {
  const targetDate = getPastDate(5);
...
const aggregateWeatherData = async () => {
  ...
  const stationId = "170"; // Replace with actual station ID if dynamic
  if (etData[stationId]) {
...
router.get("/weather", async (req, res) => {
  try {
    const aggregatedData = await aggregateWeatherData();
...
router.get("/weatherfromdb", async (req, res) => {
  try {
    ...
    const result = await request.query(
      "SELECT * FROM [2025_weather_data] ORDER BY date DESC"
    );
