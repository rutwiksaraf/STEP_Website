...
function FilterAdminPage() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`/api/getAdminCrops?adminId=${adminId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      .then((response) => {
        setAdminCrops(response.data.crops);
  ...
  const showCorn = adminCrops.includes("Corn");
  const showCotton = adminCrops.includes("Cotton");
  const username = localStorage.getItem("username");
  ...
  return (
    <div>
      <AppBar position="static" color="default">
        <Paper elevation={3}>
          <Tabs
            ...
            textColor="primary"
          >
            {adminCrops.includes("Corn") && (
              <Tab label="Corn" {...a11yProps(0)} />
            )}
            {adminCrops.includes("Cotton") && (
      ...
      {showCotton && (
        <TabPanel value={value} index={showCorn ? 1 : 0}>
          <CottonAdminPage />
        </TabPanel>
      )}
    </div>
