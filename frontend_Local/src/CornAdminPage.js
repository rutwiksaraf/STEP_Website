...
// import CornAdminPage from "./CornAdminPage.js";
...
function CornAdminPage({ showSuperAdminFeatures = false }) {
  ...
  const [deleteStatusMessagei, setDeleteStatusMessagei] = useState("");
  const [deleteStatusMessaged, setDeleteStatusMessaged] = useState("");
  const [deleteStatusMessaget, setDeleteStatusMessaget] = useState("");
    ...
    axios
      ...
      .then((response) => {
        if (response.status === 200) {
          ...
          setDeleteStatusMessaget("File deleted successfully."); // Set success message
          // Optionally, clear the message after a delay
          setTimeout(() => setDeleteStatusMessaget(""), 3000);
        }
      })
      .catch((error) => {
        // Handle error
        console.error("Error deleting file:", error);
        setDeleteStatusMessaget("Error deleting file."); // Set error message
        setTimeout(() => setDeleteStatusMessaget(""), 3000);
      });
  };
  ...
  const handleinsuranceDelete = (fileName) => {
    axios
      ...
      .then((response) => {
        if (response.status === 200) {
          // Handle successful deletion
          setDeleteStatusMessagei("File deleted successfully."); // Set success message
          // Optionally, clear the message after a delay
          setTimeout(() => setDeleteStatusMessagei(""), 3000);
        }
      })
      .catch((error) => {
        // Handle error
        console.error("Error deleting file:", error);
        setDeleteStatusMessagei("Error deleting file."); // Set error message
        setTimeout(() => setDeleteStatusMessagei(""), 3000);
      });
  };
  ...
  const handlemarketingDelete = (fileName) => {
    axios
      ...
      .then((response) => {
        if (response.status === 200) {
          // Handle successful deletion
          setDeleteStatusMessagem("File deleted successfully."); // Set success message
          // Optionally, clear the message after a delay
          setTimeout(() => setDeleteStatusMessagem(""), 3000);
        }
      })
      .catch((error) => {
        // Handle error
        console.error("Error deleting file:", error);
        setDeleteStatusMessagem("Error deleting file."); // Set error message
        setTimeout(() => setDeleteStatusMessagem(""), 3000);
      });
  };
  ...
  const handledefaultDelete = (fileName) => {
    axios
      ...
      .then((response) => {
        if (response.status === 200) {
          // Handle successful deletion
          setDeleteStatusMessaged("File deleted successfully."); // Set success message
          // Optionally, clear the message after a delay
          setTimeout(() => setDeleteStatusMessaged(""), 3000);
        }
      })
      .catch((error) => {
        // Handle error
        console.error("Error deleting file:", error);
        setDeleteStatusMessaged("Error deleting file."); // Set error message
        setTimeout(() => setDeleteStatusMessaged(""), 3000);
      });
  };
  ...
  const fetchProfileImage = () => {
    const profileImageUrlEndpoint = `/api/getProfileImageUrl/corn/${selectedUser.teamName}`;
  ...
  const handleChangeApplied1 = (appId) => {
    // Make a POST request to your Express.js route to update the "applied" field
    axios
      .post(`/api/updateCottonApplied/${appId}`, {
        headers: {
          Authorization: `Bearer ${token}`, // Include the token in the Authorization header
        },
      })
  ...
  const handleNitrogenChangeApplied1 = (appId) => {
    // Make a POST request to your Express.js route to update the "applied" field
    axios
      .post(`/api/updateCottonNitrogenApplied/${appId}`, {
        headers: {
          Authorization: `Bearer ${token}`, // Include the token in the Authorization header
        },
      })
  ...
  function handleCottonInsuranceFileUpload() {
    ...
    fetch("/api/uploadCottonInsuranceFile", {
      method: "POST",
  ...
  function handleCottonMarketingOptionsFileUpload() {
    ...
    fetch("/api/uploadCottonMarketingOptionsFile", {
      method: "POST",
  ...
  function handledefaultFileUpload() {
    ...
    // Send a POST request to upload the marketing options file
    fetch("/api/uploadCottonDefaultFile", {
      method: "POST",
      body: defaultData,
    })
      .then((response) => {
  ...
  const fetchInsuranceCottonSelectionData = () => {
    axios
      .post("/api/cottongetAllInsuranceSelectionForms", {
        headers: {
          Authorization: `Bearer ${token}`, // Include the token in the Authorization header
        },
      })
              ...
              <div>
                <div>
                  ...
                  <TabPanel value={value1} index={2}>
                    ...
                    {seedingsubmittedForms.length > 0 && selectedUser && (
                      <div>
                        <Typography variant="h5" gutterBottom>
                          Submitted Forms
                        </Typography>
                                    ...
                                    <TableCell>
                                      ...
                                      >
                                        {app.applied === "no" ? (
                                          <DeleteIcon
                                            onClick={() =>
                                              handleDeleteNitrogenApplication(
                                                app.id
                                              )
                                            }
                                          />
